...
public class BitmapButton implements ObjectTouchHandler.ITouchable {
    ...
    public void setTouchState(ObjectTouchHandler.TouchState state, int x, int y) {
        ...
        else if(ObjectTouchHandler.TouchState.Down==state)
        {
            _IsPressed = true;
        }
    }

    private void setTouchStatePressed(ObjectTouchHandler.TouchState state) {
        switch (state){
            case Cancel:
            case DragOutside:
            case UpOutside:
